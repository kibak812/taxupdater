...
### 2025.07.14 - 웹 데이터 테이블 필터링 시스템 구현
#### 🔍 필터링 기능 추가
- **사이트별 필터링**: 조세심판원(세목+유형), 국세청 유권해석(세목)
- **스마트 필터링**: 전체 데이터 기준 필터링으로 사용자 경험 개선
- **1페이지 자동 이동**: 필터 적용 시 첫 페이지부터 연속 데이터 표시

#### 🚀 이중 모드 시스템
- **일반 모드**: 서버 사이드 페이지네이션 (50개씩 로드)
- **필터 모드**: 전체 데이터 로드 후 클라이언트 사이드 필터링

#### 🎨 UI/UX 개선
- **필터 패널**: 토글 버튼으로 필터 패널 열기/닫기
- **동적 옵션**: 실제 데이터에서 필터 옵션 자동 생성
- **상태 표시**: 필터 조건과 결과 수를 명확히 표시

#### 🔧 기술적 특징
- **성능 최적화**: 필터 상태에 따른 데이터 로딩 방식 자동 전환
- **상태 관리**: `isFiltered` 플래그로 모드 구분
- **검색 연동**: 기존 검색 기능과 필터 기능 병행 지원
