...
## 📧 최신 업데이트 (2025.07.29): 이메일 알림 기능 추가

### 🎯 새로운 기능
1. **이메일 자동 알림**: 새로운 데이터 발견 시 이메일 자동 발송
2. **웹 기반 이메일 설정**: 직관적인 설정 인터페이스 제공
3. **다양한 SMTP 지원**: Gmail, Naver, Outlook 등 주요 서비스 지원
4. **테스트 이메일 기능**: 설정 검증을 위한 테스트 발송
5. **고급 재시도 로직**: 발송 실패 시 자동 재시도 (지수 백오프)
...
### 📋 사용 방법
1. `.env` 파일에 이메일 비밀번호 설정
2. 웹 설정(`/settings`)에서 이메일 구성
3. 테스트 이메일로 작동 확인
4. 자동 크롤링 시 새 데이터 발견 시 이메일 수신
